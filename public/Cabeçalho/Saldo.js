...
<script>
  ...
  function addToMega(profile, level, amount) {
    ...
    const blueFundValue = totalValue * 0.4;
    ...
    // Update balances
    balances.mega.total += totalValue;
    balances.mega.contribution += contributionValue;
    balances.mega[profile][level] += totalValue;
    
    // Add to Blue Fund from Mega
    balances.blueFund.total += blueFundValue;
    balances.blueFund.fromMega += blueFundValue;
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('add-mega-btn').addEventListener('click', () => {
      ...
      const result = addToMega(profile, level, amount);
